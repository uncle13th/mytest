...
<div class="container mx-auto px-6 py-8">
    <div class="flex items-center justify-between">
        <h3 class="text-gray-700 text-3xl font-medium">编辑菜单</h3>
            ...
            <div class="grid grid-cols-1 gap-6 mt-4 sm:grid-cols-2">
                ...
                <div>
                    ...
                    <select class="form-select w-full mt-2 rounded-md border-gray-300" 
                            ...
                            id="type" 
                            required>
                        <option value="menu" {{ old('type', $menu->type) == 'menu' ? 'selected' : '' }}>菜单</option>
                        <option value="folder" {{ old('type', $menu->type) == 'folder' ? 'selected' : '' }}>文件夹</option>
                    </select>
                    @error('type')
                        <p class="text-red-500 text-xs mt-1">{{ $message }}</p>
                    @enderror
                ...
                <div>
                    ...
                    <select class="form-select w-full mt-2 rounded-md border-gray-300" 
                            ...
                            id="is_show" 
                            required>
                        <option value="1" {{ old('is_show', $menu->is_show) == 1 ? 'selected' : '' }}>显示</option>
                        <option value="0" {{ old('is_show', $menu->is_show) == 0 ? 'selected' : '' }}>隐藏</option>
                    </select>
                    @error('is_show')
                        <p class="text-red-500 text-xs mt-1">{{ $message }}</p>
                    @enderror
